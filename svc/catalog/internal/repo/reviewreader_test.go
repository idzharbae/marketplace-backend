...
func TestReviewReader_GetTotalAndAverageByShopID(t *testing.T) {
	test := newReviewReaderTest()
	t.Run("db returns error, should return error", func(t *testing.T) {
		test.Begin(t)
		defer test.Finish()
		...
		test.db.EXPECT().Select("COUNT(*) AS total, AVG(rating) AS average").Return(test.db)
		test.db.EXPECT().Where("shop_id=?", req).Return(test.db)
		test.db.EXPECT().Find(gomock.Any()).Return(test.db)
		test.db.EXPECT().Error().Return(errors.New("error"))

		got, err := test.unit.GetTotalAndAverageByShopID(req)
		assert.NotNil(t, err)
		assert.Equal(t, int32(0), got.Total)
		assert.Equal(t, float32(0), got.Average)
	})
	t.Run("db returns no error, should return total and average", func(t *testing.T) {
		test.Begin(t)
		defer test.Finish()
		req := int64(1234)
		...
		test.db.EXPECT().Select("COUNT(*) AS total, AVG(rating) AS average").Return(test.db)
		test.db.EXPECT().Where("shop_id=?", req).Return(test.db)
		test.db.EXPECT().Find(gomock.Any()).DoAndReturn(func(arg *requests.TotalAndAverageReview) *gormmock.MockGormw {
			*arg = requests.TotalAndAverageReview{Average: 123, Total: 321}
			return test.db
		})
		test.db.EXPECT().Error().Return(nil)
		...
		got, err := test.unit.GetTotalAndAverageByShopID(req)
		assert.Nil(t, err)
		assert.Equal(t, int32(321), got.Total)
		assert.Equal(t, float32(123), got.Average)
	})
}

func TestReviewReader_GetTotalAndAverageByProductID(t *testing.T) {
	test := newReviewReaderTest()
	t.Run("db returns error, should return error", func(t *testing.T) {
		test.Begin(t)
		defer test.Finish()
		...
		got, err := test.unit.GetTotalAndAverageByProductID(req)
		assert.NotNil(t, err)
		assert.Equal(t, int32(0), got.Total)
		assert.Equal(t, float32(0), got.Average)
	})
